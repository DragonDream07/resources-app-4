...
export default function CategoryProductListing() {
  ...
  // Resolve category by slug via categories list
  const fetchCategory = useCallback(async () => {
    ...
    try {
      ...
      setCategory(found);
      setCategoryId(found.id || found.categoryId);
    } catch (err) {
  ...
  }, [categoryId, currentPage, currentSort, minPrice, maxPrice, selectedBrands.join(',')]);
  ...
  const toggleBrand = (brandId) => {
    ...
    next.delete('brand');
    if (existing.includes(String(brandId))) {
      existing.filter((v) => v !== String(brandId)).forEach((v) => next.append('brand', v));
    } else {
      [...existing, String(brandId)].forEach((v) => next.append('brand', v));
    }
    next.set('page', '1');
    setSearchParams(next);
  };
    ...
    return (
      <div style={styles.page}>
        <div style={styles.container}>
          <div style={styles.loadingText}>Loading category…</div>
        </div>
      </div>
    );
  }
  ...
  if (catError) {
    return (
      <div style={styles.page}>
        <div style={styles.container}>
          <div style={styles.notFoundBox}>{catError}</div>
        </div>
      </div>
    );
  }
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>{category?.name || slug}</h1>
          {category?.description && (
            <p style={styles.description}>{category.description}</p>
          )}
          <p style={styles.resultCount}>
            {loading ? 'Loading…' : <><strong>{totalCount.toLocaleString()} products</strong> in this category</>}
          </p>
        </div>
                ...
                {brands.map((brand) => (
                  <label key={brand.id || brand.brandId} style={styles.filterOption}>
                    <input
                      ...
                      onChange={() => toggleBrand(brand.id || brand.brandId)}
                    />
                    {brand.name}
                  </label>
                ))}
