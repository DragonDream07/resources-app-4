...
export default function AdminCategoryEdit() {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      try {
        const [catRes, selfRes] = await Promise.all([
          fetch('/categories', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }),
          fetch(`/categories/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }),
        ]);
        if (!selfRes.ok) throw new Error('Category not found.');
        const catData = await catRes.json();
        const selfData = await selfRes.json();
        if (!cancelled) {
          setCategories((catData.categories ?? catData.data ?? catData ?? []).filter(c => String(c.id) !== String(id)));
          const cat = selfData.category ?? selfData;
          setForm({
            name: cat.name ?? '',
            slug: cat.slug ?? '',
  ...
  if (loading) {
    return (
      <div style={styles.page}>
        <div style={styles.header}><h1 style={styles.title}>Edit Category</h1></div>
  ...
  if (loadError) {
    return (
      <div style={styles.page}>
        ...
        <Link to="/admin/catalogue/categories" style={styles.cancelBtn}>Back to Categories</Link>
      </div>
    );
  }
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div style={styles.actions}>
            ...
            <Link to="/admin/catalogue/categories" style={styles.cancelBtn}>Back</Link>
          </div>
        </form>
      </div>
    </div>
